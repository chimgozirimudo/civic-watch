...
const readJsonResponse = async (response) => {
  ...
  try {
    ...
    throw new Error('The backend did not return valid data. Please restart the backend server and try again.');
  }
};
...
export default function ReportDetail() {
  ...
  const hasMapPin = report?.latitude !== null && report?.latitude !== undefined && report?.longitude !== null && report?.longitude !== undefined;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 px-6 py-16 text-center">
        ...
        <p className="text-sm font-semibold text-slate-600">Loading report details...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 px-6 py-16">
        <div className="max-w-xl mx-auto text-center bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
          ...
          <h1 className="text-xl font-extrabold text-slate-900">Report unavailable</h1>
    ...
    <div className="min-h-screen bg-slate-50 pb-16">
      <div className="bg-slate-900 text-white border-b border-slate-800 py-8 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-5">
            <div>
              <p className="text-xs font-bold text-blue-400 mb-2">Report #{report.id}</p>
              <h1 className="text-3xl font-extrabold tracking-tight">{report.title}</h1>
              <p className="text-sm text-slate-400 mt-2 max-w-2xl">{report.description}</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <span className={`inline-flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-full border bg-white ${getStatusBadgeColor(report.status)}`}>
                {getStatusIcon(report.status)}
                {report.status || 'Pending'}
              </span>
              <span className={`inline-flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-full border bg-white ${getPriorityBadgeColor(report.priority)}`}>
      ...
      <main className="max-w-6xl mx-auto px-6 -mt-5 grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
              ...
              <div className="flex items-center gap-3 p-4 rounded-xl bg-slate-50 border border-slate-200">
                ...
                <div>
                  ...
                  <p className="font-bold text-slate-900">{formatDate(report.created_at)}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 p-4 rounded-xl bg-slate-50 border border-slate-200">
                <HiUsers className="w-5 h-5 text-blue-600 shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-slate-500">Citizen Support</p>
                  <p className="font-bold text-slate-900">{report.support_count || 0} supporters</p>
                </div>
              </div>
            </div>
            ...
            {report.image ? (
              ...
              <div className="mt-6 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
                ...
                <p className="text-sm font-semibold text-slate-600">No image was attached to this report.</p>
              </div>
            )}
        ...
        <aside className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm self-start">
          <h2 className="text-lg font-extrabold text-slate-900 mb-5">Status Timeline</h2>
          <div className="space-y-4">
            {timeline.map((event, index) => (
              <div key={`${event.id}-${index}`} className="relative pl-8">
                ...
                <p className="text-xs text-slate-500 mt-1">{formatDate(event.created_at)}</p>
              </div>
            ))}
          </div>
        </aside>
